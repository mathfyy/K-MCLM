...
import torch
import pandas as pd
import K_MCLM.evaluation_metrics as evaluation_metrics
...
def test_model_K_MCLM(CNN, csc, class_token, Ag_param, task_id, label_num):
    for index, (image_ids, inputs, labels, names) in enumerate(test_loader):
        ...
        with torch.set_grad_enabled(False):
            ...
            flops_base, params_base = profile(CNN, inputs=(inputs,))
            print(task_id, flops_base / 1e9, params_base / 1e6)
            flops_csc, params_csc = profile(csc, inputs=(class_token, F, Ag_param,))
            print(task_id, flops_csc / 1e9, params_csc / 1e6)
...
from K_MCLM.resnet3D import resnet101, cscBlock
...
if __name__ == '__main__':
    ...
    model_type = 'K_MCLM'
    data_path = '/data1/Data/GBM/BrainTumorGene/CL_gene_data/'
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")

    model_path = data_path + '/result_R/' + split_type + '/' + model_type + '/'
    ...
    for task_id in range(num_tasks):
        ...
        for i in range((int(task_id) + 1)):
            ...
            csc.load_state_dict(torch.load(model_path + '/csc_model' + str(task_id) + '.pt'), strict=False)
            ...
            acc, acc_P, acc_N, mauc, mmcc, muf1, muar = test_model_K_MCLM(CNN, csc, class_token, Ag_param, i,
                                                                            (i + 1) * num_classes)
