...
data_path = '/data1/Data/GBM/BrainTumorGene/CL_gene_data/pretrain_model/'
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer2 = self._make_layer(
            block, 128, layers[1], shortcut_type, stride=(2, 2, 2))
        self.layer3 = self._make_layer(
            block, 256, layers[2], shortcut_type, stride=(1, 2, 2), dilation=2)
        self.layer4 = self._make_layer(
            block, 512, layers[3], shortcut_type, stride=(1, 1, 1), dilation=4)

        # 最佳 avg 64%
        self.maxpool2 = nn.MaxPool3d((7, 7, 7), (7, 7, 7))
...
class cadBlock(nn.Module):
    def __init__(self, device, mid_dim=2048):
        super(cadBlock, self).__init__()
        self.mid_dim = mid_dim
        self.device = device
        self.ln_F_data = nn.Linear(mid_dim, 512)
        self.ln_A_cls = nn.Linear(mid_dim, 512)
...
class cscBlock(nn.Module):
    def __init__(self, device, D0, D1, D2, mid_dim=2048):
